 ...
 */
void
ILI9341_DrawRectangle(uint16_t usX_Start, uint16_t usY_Start, uint16_t usWidth, uint16_t usHeight, uint8_t ucFilled) {
    if (ucFilled) {
...
void ILI9341_DispChar_CH(uint16_t usX, uint16_t usY, uint16_t *usChar) {
    ...
    //printf("ILI9341_DispChar_CH:0x%x\n",*usChar);
    //printf("ILI9341_DispChar_CH:0x%x\n",*(usChar+1));
    //printf("ILI9341_DispChar_CH:0x%x\n",*(usChar+2));
    //取字模数据
    GetUTF8Code(ucBuffer, usChar);
...
void ILI9341_DispString_CH(uint16_t usX, uint16_t usY, uint16_t *pStr) {
    ...
    while (*pStr != '\0') {
        ...
        ILI9341_DispChar_CH(usX, usY, pStr);
 ...
 * @retval 无
 */
void ILI9341_DispStringLine_EN_CH(uint16_t line, uint16_t usX, char *pStr) {
    uint16_t usCh;
    //uint16_t usX = 0;
    int i = 0;
    while (*pStr != '\0') {
        printf("ILI9341_DispStringLine_EN_CH1:0x%x\n", *pStr);
        if (*pStr <= 126)                //英文字符
        {
            if ((usX - ILI9341_DispWindow_X_Star + LCD_Currentfonts->Width) > LCD_X_LENGTH) {
                usX = ILI9341_DispWindow_X_Star;
        ...
        } else                                //汉字字符
        {
            ...
            //     for (i = 0; i < wcslen(pStr); i++)
            //		{
            //printf("aaaILI9341_DispStringLine_EN_CH:0x%x\n",*(pStr+i));
            //  }

            //printf("ILI9341_DispStringLine_EN_CH:0x%x\n",*pStr);
            //printf("ILI9341_DispStringLine_EN_CH:0x%x\n",*(pStr+1));
            //printf("ILI9341_DispStringLine_EN_CH:0x%x\n",*(pStr+2));
            //usCh = * ( uint16_t * ) pStr;
